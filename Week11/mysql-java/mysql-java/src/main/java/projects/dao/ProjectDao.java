...
public class ProjectDao extends DaoBase {
   ...
   * table. It does not
   * retrieve any materials, steps, or categories. The project rows are ordered by
   * project name.
   * 
  ...
  public Optional<Project> fetchProjectById(Integer projectId) {
    ...
    try (Connection conn = DbConnection.getConnection()) {
       ...
       * accompanying
       * materials, steps and categories so that, if an error occurs at any place, the
       * transaction
      ...
      try {
         ...
         * each method
         * will simply return an empty list. However, it avoids three unnecessary
         * database calls.
         */
   ...
   * ID. Note the inner
   * join to join the category rows to the project_category join table. The join
   * table is needed
   * because projects and categories have a many-to-many relationship. Categories
   * can exist on their
   * own without having associated projects and projects can exist on their own
   * without having any
   ...
   * project ID. The
   * connection is supplied by the caller so that steps can be retrieved on the
   * current transaction.
   * 
   ...
   * given project ID. The
   * connection is supplied by the caller so that project materials can be
   * retrieved on the current
   * transaction.
   * 
   * @param conn      The caller-supplied connection.
